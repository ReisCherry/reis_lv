...
function checkRealign(key, near) {
  ...
  if (key.startsWith('mob:')) {
    ...
    timeoutMs = 30000; // 30 seconds max without any progress
    minAttacks = 30; // 30 swings without progress means we are stuck or out of range
  }
...
function getBestFarmingOffset(rx, ry, failedSet) {
  ...
  if (chunks.landTiles && chunks.landTiles.size > 0) {
    for (const off of offsets) {
      if (failedSet && failedSet.has(`${off.dx},${off.dy}`)) continue;
