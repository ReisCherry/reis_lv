...
function findBestResource(type) {
  ...
  if (type === 'tree') {
    ...
    if (candidateList.length > 0) {
      ...
      const prefOffsetX = Math.cos(angle) * 150;
      const prefOffsetY = Math.sin(angle) * 150;
      ...
      for (const item of candidateList) {
        ...
        // Score: higher density is better, lower distance is better
        // density weight + distance penalty
        const score = (density * 50) - d;
        if (score > bestScore) {
          bestScore = score;
